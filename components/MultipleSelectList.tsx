...
const MultipleSelectList: React.FC<MultipleSelectListProps> = ({
  ...
  const oldOptions = React.useRef(null);
  ...
  React.useEffect(() => {
    ...
    const haveSameOptions = (
      ...
      return (
        ...
        options1.every((element, index) => element.key === options2[index].key)
      );
    };
    ...
    if (
      !_firstRender &&
      defaultOptions &&
      !haveSameOptions(oldOptions.current, defaultOptions)
    ) {
      const values = defaultOptions.map((element: Option) => element.value);
      oldOptions.current = defaultOptions;
      if (save === "value") {
        setSelected(values);
      } else {
        setSelected(defaultOptions.map((element: Option) => element.key));
      }
